test_that("trawl_sup_ig_error", {
  params <- c(0.1, 1)
  h_max <- 50
  h_collection <- seq_len(h_max)
  one_vals <- vapply(h_collection, function(h) {
    sup_ig_trawl$trawl_b_one(params, h)
  }, 1)
  testthat::expect_error(sup_ig_trawl$trawl_b_one(-c(params, params), h))
  testthat::expect_error(sup_ig_trawl$trawl_b_two(-c(params, params), h))
  testthat::expect_error(sup_ig_trawl$trawl_b_three(-c(params, params), h))
})

test_that("trawl_sup_ig_one", {
  params <- c(0.1, 1)
  h_max <- 50
  h_collection <- seq_len(h_max)
  one_vals <- vapply(h_collection, function(h) {
...
test_that("trawl_sup_ig_completement", {
  params <- c(0.1, 1)
  h_max <- 50
  h_collection <- seq_len(h_max)
  ...
  testthat::expect_equal(
    should_equal_to_one, rep(params[1] / params[2], h_max),
    tolerance = 1e-3
  )
})

test_that("trawl_sup_ig_cfg", {
  cfg <- sup_ig_trawl$config()
