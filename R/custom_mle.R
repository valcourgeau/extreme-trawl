CustomLikelihood <- function(data){
  ...
  return(function(par){
    ...
    like <- evir::dgpd(data_for_mle[data_for_mle>0.0],
                       beta = par[2], xi=par[1], mu=0.0) * p_non_zero
...
CustomMarginalMLE <- function(data, parametrisation='standard'){
  length_data <- min(20000, length(data))
  init_guess <- as.numeric(evir::gpd(data[1:length_data], threshold = 0)$par.ests)
  cat('custom MLE init_guess', init_guess, '\n')
